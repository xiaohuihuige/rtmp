...
static void _buildOnMetaData(bs_t *b, StreamConfig *stream);
...
static void _buildSampleAccess(bs_t *b,  double stream_id, const char *sample_access)
{
    ...
    amf_write_boolean(b, BOOLEAN_TRUE);
    amf_write_boolean(b, BOOLEAN_TRUE);
}

static void _buildVideoOnMetaData(bs_t *b, VideoConfig *video)
{
    amf_write_NamedDouble(b, "width", strlen("width"), video->width);
    amf_write_NamedDouble(b, "height", strlen("height"), video->height);
    amf_write_NamedDouble(b, "displayWidth", strlen("displayWidth"), video->displayWidth);
    amf_write_NamedDouble(b, "displayHeight", strlen("displayHeight"), video->displayHeight);

    amf_write_NamedDouble(b, "framerate", strlen("framerate"), video->fps);
    amf_write_NamedDouble(b, "fps", strlen("fps"), video->fps);
    amf_write_NamedDouble(b, "videodatarate", strlen("videodatarate"), video->videodatarate);
    amf_write_NamedDouble(b, "videocodecid", strlen("videocodecid"), video->videocodecid);
}

static void _buildAudioOnMetaData(bs_t *b, AudioConfig *audio)
{
    ...
    amf_write_NamedBoolean(b, "stereo", strlen("stereo"), BOOLEAN_TRUE);
}
...
static void _buildOnMetaData(bs_t *b, StreamConfig *stream)
{
    ...
    if (0) {
        length += AMF_NAMEDOUBLE_LENGTH("width") +
    ...
    if (stream->enable_audio) {
        length += AMF_NAMEDOUBLE_LENGTH("audiodatarate") +
    ...
    amf_write_NamedDouble(b, "duration", strlen("duration"), stream->duration);
    amf_write_NamedString(b, "profile", strlen("profile"), "", stream->profile);
    amf_write_NamedString(b, "level", strlen("level"), "", stream->level);

    if (0)
        _buildVideoOnMetaData(b, stream->video);

    if (stream->enable_audio)
        _buildAudioOnMetaData(b, stream->audio);
...
// Property 'audiocodecid' Number 2
// Property 'minor_version' String '512'
// Property 'major_brand' String 'isom'
// Property 'compatible_brands' String 'isomiso2avc1mp41'
...
int sendOnMetaData(RtmpSession *session, Buffer *buffer)
{
    ...
    _buildOnMetaData(b, &session->stream);
